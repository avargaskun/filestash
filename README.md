...
| `application_video.js`, `application_video.css` | Usable fullscreen on a phone. Upstream fullscreens the bare `<video>`, so the overlay control bar — a sibling of the element — is left outside the fullscreen subtree and the only controls are tap-to-play/pause and the system back gesture. Fullscreen now targets the whole player page, keeping the seek bar, volume, timecode **and the quality menu** available; the menubar button becomes a real enter/exit toggle driven by `fullscreenchange` (so Esc and the back gesture can never desync it, and it stays absent where there is no fullscreen API). Adds ±10 s skip buttons (40 px touch targets, inline and fullscreen, hidden while casting), auto-hide of the controls after 3 s of fullscreen playback with tap-to-reveal that does **not** also pause, and a landscape orientation lock on enter to replace the rotation that media-element fullscreen gave for free. Two upstream bugs fixed on the way: the control bar was invisible under `prefers-reduced-motion: reduce` (its resting CSS is `opacity: 0` and only a skipped animation made it visible — now a class over the existing transition), and seeking while `duration` is `NaN` (i.e. during every quality switch) threw into the global error screen — both seek paths now guard for finiteness | 1.3.0 |
| `plg_video_transcoder` (both encode variants) | Declare the codecs the segments actually carry. The master playlist hardcoded `CODECS="avc1.64001f,mp4a.40.2"` (and, on the ffmpeg variant, an audio rendition group), so a video-only source — a camera clip, a muted export — made MSE/hls.js open an audio buffer that no segment could ever fill: playback stalled after segment 0 with no error anywhere. The source-duration probe now reports audio-stream presence alongside the duration in the same probe, and an audio-less source gets `CODECS="avc1.64001f"`, no `EXT-X-MEDIA` rendition and a bandwidth figure without the audio budget. Audio-bearing sources are byte-identical to before, and a source that cannot be probed keeps the historical shape. Tests: `server/plugin/plg_video_transcoder/ffmpeg/handler_test.go` (in CI) and `libav/handler_test.go` (needs cgo + the libav headers) | 1.3.1 |
| `plg_video_transcoder/libav` | Survive HLS segments whose window holds no video frame, and end playback cleanly. A final "sliver" segment (or any out-of-range segment index) whose window contains no in-window video frame left the lazily-built video filter graph NULL, and the flush loop's `push_filter(s, NULL)` dereferenced it → SIGSEGV **inside cgo that killed the whole server process** — a one-request remote process-kill reachable with only a valid opaque mediaref key. The `push_filter` guard (`!s->src \|\| !s->sink` ⇒ nothing to flush) covers every present/future call site, and a non-negative `FF_SEGMENT_NO_FRAMES` sentinel (Go's negative=error contract untouched) makes the outcome greppable at INFO, not a fake ERROR at every end-of-playback. When that zero-video-frame window also leaves the muxer with nothing to emit at all (both streams empty), an empty 200 body sends hls.js into a fatal parse-error retry storm (7 needless server re-encodes) that never fires `ended`; so Go substitutes a tiny embedded valid MPEG-TS (`minimal.ts`, h264+aac, real samples) **only when the muxer wrote zero bytes** — a degenerate window that produced a valid audio-only TS is left untouched. Both shapes now reach a clean `BUFFER_EOS`/`ended` in hls.js. Tests: `server/plugin/plg_video_transcoder/libav/transcode_test.go` (needs cgo + the libav headers; asserts the degenerate path is non-empty and the out-of-range path is the embedded TS) | 1.3.2 |
| `plg_video_transcoder/libav` | Skip the encoder EOF flush for a segment that never fed the encoder. The 1.3.2 guard stopped the NULL filter-graph deref, but the flush loop still called `avcodec_send_frame(s->enc, NULL)` on an opened-but-never-fed encoder — legal libavcodec API use that libavcodec's own hardware-encoder base layer NULL-derefs: `hw_base_encode.c:507` reads `ctx->pic_end->pts` under an `input_order <= decode_delay` test that is true when `input_order == 0`, while `pic_end` is only ever assigned on a non-NULL frame. So under `h264_vaapi` — the production encoder — every zero-video-frame segment still killed the whole server process (`si_addr = 0x28`, gdb-confirmed on an Intel UHD 770 against the published 1.3.2 image). libx264 is a plain `encode2` encoder with no such path, which is why the libx264-only gate passed. A per-stream `frames_enc` counter, incremented at the one site that can ever feed the encoder (`push_filter`), now skips both the EOF flush and its drain when the encoder received nothing — an encoder that was never fed has nothing buffered to flush either. Encoder-agnostic (the API call is removed on that path, so no backend-specific code runs), decided per stream, and inert for any segment that produced a frame. The decoder flush stays unconditional: it is what produces the last frames of a file. The deref is still live in FFmpeg `master` (`hw_base_encode.c:511-512`) and in released `n8.0` (`:507-508`), so the guard is permanent rather than a bridge until Debian bumps FFmpeg | 1.3.3 |
| `plg_video_transcoder` (new `hlsmath` pkg, `libav`, `ffmpeg`) | Clamp the HLS playlist to the media's real content end. The playlist was generated from the **container metadata duration**, which is not a promise that content exists: a 24-minute episode from a mainstream release group declares 1484.050 s while both its streams end at 1430.09 s, so the playlist advertised 297 segments of which the last 10 held nothing at all. The client's `duration` is the playlist's `#EXTINF` sum, so the seek bar, the ±10 s buttons and every percentage seek mapped into a region with no content — and each request there hit the 1.3.2/1.3.3 zero-frame path and got a substituted 4512 B TS whose baked-in ~0 s timestamps never advance a native player's clock (observed: black screen, then an infinite spinner). Both variants now measure the real end — `max(last video PTS, last audio PTS)` over the streams `av_find_best_stream` picks, i.e. exactly the ones that get transcoded — and emit `ceil(min(container_duration, content_end) / segment_length)` segments. The scan is packet-level (no decode), starts 10 s before the declared end and backs off to 90 s then 600 s, and **fails open to the container duration on anything unexpected**: it is capped at 256 MB cumulative and 500k packets checked inside the read loop (a bound expressed in seconds of the *declared* duration is no bound at all when that duration is `AV_NOPTS_VALUE` or is itself truncated on a still-downloading file), it accepts a result **only** on a clean `AVERROR_EOF` (otherwise a corrupt file would have its playlist truncated to the damage point), it refuses to run unless the declared duration is finite and positive, and an `AVIOInterruptCB` wired to the request context lets a client disconnect stop it. PTS are taken as a **running maximum** — packets arrive in decode order, so the last packet read is routinely not the largest — and are **not** normalised by `start_time`, matching the transcoder's own window arithmetic. The clamp can only ever shorten a playlist, never lengthen one, and never fails a request. The three hand-rolled playlist loops moved into a shared `hlsmath` package added to CI's test line, with a byte-identity test against the pre-extraction output. Tests: `server/plugin/plg_video_transcoder/hlsmath/hlsmath_test.go`, run in CI | 1.3.4 |
